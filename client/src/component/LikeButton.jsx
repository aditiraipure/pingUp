...
const LikeButton = ({ liked, count, onClick, disabled = false, vertical = false, compact = false, ariaLabel = "Like" }) => (
  <button
    type="button"
    onClick={onClick}
    disabled={disabled}
  ...
  >
    ...
    {count !== undefined && count !== null && <span className={`${compact ? "text-[9px] leading-3" : "text-xs"} text-gray-500`}>{count}</span>}
  </button>
);
