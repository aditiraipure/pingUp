...
const SharePostModal = ({ post, story, onClose, onShared }) => {
  ...
  const shareWith = async (friend) => {
    try {
      ...
      if (post) {
        const shareResponse = await api.post(`/api/post/${post._id}/share`, {}, {
          headers: { Authorization: `Bearer ${await getToken()}` },
        });
        if (shareResponse.data.success) onShared?.(shareResponse.data.share_count);
      }
